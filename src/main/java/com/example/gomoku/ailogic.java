...
public class ailogic {
    ...
    private void findNeighborPositions(Set<String> pieces, Set<String> neighborPositions) { // O(n)
        for (String piece : pieces) {
            ...
            int col = Integer.parseInt(moves[1]);
            int newRow, newCol;
            for (int[] dir : directions) {//adds to neighborpositions all of the empty positions around the pieces
                newRow = row + dir[0];
                newCol = col + dir[1];
                if (isValidMove(newRow, newCol)) {
    ...
    private int[] bestMove(Set<String> neighborPositions) { // searching for the best move in the neighbor set
        Set<String> blackMoves = new HashSet<>(gameLogic.getBlackMoves());
        int[] bestMove = null; // best move to be returned
        String bestPattern = null;
        for (String position : neighborPositions) { // check every string in the neighbors
            String[] moves = position.split(",");
            int moveRow = Integer.parseInt(moves[0]);
            int moveCol = Integer.parseInt(moves[1]);
            String currentPattern = null;
            if (wouldCreatePattern(moveRow, moveCol, blackMoves, "DeadFour")) currentPattern = "DeadFour";// checks what pattern is created on each position
    ...
    private boolean wouldCreatePattern(int row, int col, Set<String> playerMoves, String targetPattern) {
        Set<String> tempPlayerMoves = new HashSet<>(playerMoves);// new one so we can add and change this
